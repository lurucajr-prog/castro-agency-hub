...
export default function Dashboard({ user, setPage }) {
  ...
  async function fetchAll() {
    ...
    setQuoteText(s.find(x => x.key === 'quote_text')?.value || 'Make today count.')
    setQuoteAuthor(s.find(x => x.key === 'quote_author')?.value || '')
    setLoading(false)
  }
          ...
          <div style={{ background: N, borderRadius: 10, padding: '14px 16px', position: 'relative' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              {isAdmin && !editingQuote && (
                <button onClick={() => { setQuoteDraft({ text: quoteText, author: quoteAuthor }); setEditingQuote(true) }} style={{ background: 'rgba(255,255,255,0.12)', border: '0.5px solid rgba(255,255,255,0.2)', borderRadius: 5, color: 'rgba(255,255,255,0.7)', fontSize: 10, fontWeight: 500, padding: '3px 9px', cursor: 'pointer' }}>✏ Edit</button>
              )}
            </div>
            {editingQuote ? (
              <div>
                <textarea value={quoteDraft.text} onChange={e => setQuoteDraft(d => ({ ...d, text: e.target.value }))} placeholder="Enter today's quote…" rows={3} style={{ width: '100%', background: 'rgba(255,255,255,0.12)', border: '0.5px solid rgba(255,255,255,0.25)', borderRadius: 7, color: '#fff', fontSize: 13, padding: '8px 10px', resize: 'none', outline: 'none', marginBottom: 8, fontFamily: 'inherit', lineHeight: 1.6 }} />
                ...
                <div style={{ display: 'flex', gap: 7 }}>
                  <button onClick={saveQuote} disabled={savingQuote} style={{ flex: 1, background: '#fff', border: 'none', borderRadius: 7, color: N, fontSize: 12, fontWeight: 600, padding: '7px 0', cursor: 'pointer' }}>{savingQuote ? 'Saving…' : 'Save quote'}</button>
