...
function getBusinessDaysLeft() {
  ...
  while (d <= lastDay) { if (d.getDay() !== 0 && d.getDay() !== 6) count++; d.setDate(d.getDate() + 1) }
  return count
}
...
function launchConfetti() {
  const canvas=document.createElement('canvas'); canvas.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:9999'; document.body.appendChild(canvas); canvas.width=window.innerWidth; canvas.height=window.innerHeight
  const ctx=canvas.getContext('2d'); const colors=['#C8102E','#1B3A6B','#FFD700','#16a34a','#f97316','#8b5cf6','#ec4899']
  const particles=Array.from({length:220},()=>({x:Math.random()*canvas.width,y:-20-Math.random()*100,w:Math.random()*14+5,h:Math.random()*7+4,color:colors[Math.floor(Math.random()*colors.length)],speed:Math.random()*5+2,angle:Math.random()*360,spin:(Math.random()-.5)*10,drift:(Math.random()-.5)*3,opacity:1}))
  let frame=0
  function animate() { ctx.clearRect(0,0,canvas.width,canvas.height); particles.forEach(p=>{p.y+=p.speed;p.x+=p.drift;p.angle+=p.spin;if(frame>150)p.opacity=Math.max(0,p.opacity-.013);ctx.save();ctx.globalAlpha=p.opacity;ctx.translate(p.x,p.y);ctx.rotate(p.angle*Math.PI/180);ctx.fillStyle=p.color;ctx.fillRect(-p.w/2,-p.h/2,p.w,p.h);ctx.restore()}); frame++; if(frame<250)requestAnimationFrame(animate); else if(canvas.parentNode)document.body.removeChild(canvas) }
  animate()
}
...
function GoalToast({ onDone }) {
  ...
  return(
    <div style={{position:'fixed',top:80,left:'50%',transform:'translateX(-50%)',background:'#fff',border:'2px solid #16a34a',borderRadius:16,padding:'22px 40px',zIndex:9998,textAlign:'center',boxShadow:'0 20px 60px rgba(0,0,0,0.2)',animation:'toastIn .4s ease',minWidth:320}}>
...
export default function Dashboard({ user, setPage }) {
  ...
  async function fetchAll() {
    ...
    setQuoteText(getS('quote_text')||'Make today count.'); setQuoteAuthor(getS('quote_author')||'')
    setAnnouncementText(getS('announcement_text')||''); setAnnouncementActive(getS('announcement_active')==='true')
    setStandupEnabled(getS('standup_enabled')!=='false')
    if(myStandup.data) setMyMoodToday(myStandup.data.mood)
    const tally={fire:0,neutral:0,tired:0}; (allStandup.data||[]).forEach(r=>{if(r.mood==='fire')tally.fire++;else if(r.mood==='neutral')tally.neutral++;else if(r.mood==='tired')tally.tired++}); setStandupTally(tally)
    setData({ tasks:tasks.data||[], monthSales:monthSalesRes.data||[], allSales:allSalesRes.data||[], revs:revs.data||[], profiles:profiles.data||[], goals:goals.data||[] })
    setLoading(false)
  }
  ...
  useEffect(() => {
    ...
    const teamGoal=Number(50000) // could fetch from settings but keeping simple
    if(totalPrem>=teamGoal&&teamGoal>0) { goalCelebrated.current=true; launchConfetti(); setShowGoalToast(true) }
  }, [data])

  async function saveQuote() {
    if(!quoteDraft.text.trim()) return; setSavingQuote(true)
    await Promise.all([supabase.from('settings').upsert({key:'quote_text',value:quoteDraft.text.trim()}),supabase.from('settings').upsert({key:'quote_author',value:quoteDraft.author.trim()})])
    setQuoteText(quoteDraft.text.trim()); setQuoteAuthor(quoteDraft.author.trim()); setEditingQuote(false); setSavingQuote(false)
  }

  async function saveAnnouncement() {
    await Promise.all([supabase.from('settings').upsert({key:'announcement_text',value:announcementDraft}),supabase.from('settings').upsert({key:'announcement_active',value:'true'})])
    setAnnouncementText(announcementDraft); setAnnouncementActive(true); setAnnouncementDismissed(false); setEditingAnnouncement(false)
  }

  async function clearAnnouncement() { await supabase.from('settings').upsert({key:'announcement_active',value:'false'}); setAnnouncementActive(false) }
  ...
  async function submitStandup(mood) {
    ...
    setMyMoodToday(mood); setStandupTally(t=>({...t,[mood]:t[mood]+1}))
  }

  async function toggleStandup() { const n=!standupEnabled; await supabase.from('settings').upsert({key:'standup_enabled',value:String(n)}); setStandupEnabled(n) }
  ...
  const lbData=members.map(m=>({m,prem:monthSales.filter(s=>s.uid===m.id).reduce((s,x)=>s+(x.premium||0),0),items:monthSales.filter(s=>s.uid===m.id).length,streak:calcStreak(allSales,m.id)})).sort((a,b)=>b.prem-a.prem)
    ...
    <div style={{padding:22}}>
      {announcementActive&&announcementText&&!announcementDismissed&&(
        <div style={{background:N,color:'#fff',borderRadius:9,padding:'10px 16px',marginBottom:14,display:'flex',justifyContent:'space-between',alignItems:'center',gap:12}}>
          <div style={{display:'flex',alignItems:'center',gap:9}}><span style={{fontSize:16}}>📢</span><span style={{fontSize:13,fontWeight:500}}>{announcementText}</span></div>
      ...
      {isAdmin&&(!announcementActive||announcementDismissed)&&(
        <div style={{marginBottom:14}}><button onClick={()=>{setAnnouncementDraft('');setEditingAnnouncement(true)}} style={{fontSize:11,color:'#6b7280',background:'none',border:'1px dashed #d1d5db',borderRadius:7,padding:'5px 13px',cursor:'pointer'}}>+ Post announcement to team</button></div>
      )}
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:14}}>
        {stats.map(s=>(
          <div key={s.label} style={{background:s.bg,borderRadius:10,padding:'12px 14px'}}>
            <div style={{fontSize:10,color:s.tx,fontWeight:500,opacity:.7,marginBottom:3}}>{s.label}</div>
            <div style={{fontSize:22,fontWeight:500,color:s.tx,lineHeight:1,marginBottom:2}}>{s.val}</div>
            <div style={{fontSize:10,color:s.tx,opacity:.65,marginBottom:s.pct!==null?5:0}}>{s.sub}</div>
            {s.pct!==null&&(<div style={{height:3,background:'rgba(0,0,0,.1)',borderRadius:99}}><div style={{width:s.pct+'%',height:'100%',background:s.bar,borderRadius:99}}/></div>)}
          </div>
        ))}
      </div>
        ...
        <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:10,padding:'12px 16px',marginBottom:14}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:myMoodToday?4:isAdmin?8:10}}>
            ...
            <div style={{display:'flex',gap:8,alignItems:'center'}}>
              {isAdmin&&<div style={{fontSize:12,color:'#6b7280'}}>🔥 {standupTally.fire} · 😐 {standupTally.neutral} · 😴 {standupTally.tired}</div>}
              {isAdmin&&<button onClick={toggleStandup} style={{fontSize:10,color:'#6b7280',background:'none',border:'1px solid #e5e7eb',borderRadius:5,padding:'2px 8px',cursor:'pointer'}}>Turn off</button>}
            </div>
          </div>
          {!isAdmin&&(myMoodToday?(
            ...
            <div>
              ...
              <div style={{display:'flex',gap:9}}>
                {[{key:'fire',label:'🔥 Fired up'},{key:'neutral',label:'😐 Neutral'},{key:'tired',label:'😴 Low energy'}].map(opt=>(
                  <button key={opt.key} onClick={()=>submitStandup(opt.key)} style={{flex:1,padding:'8px 0',border:'1px solid #e5e7eb',borderRadius:8,background:'#f9fafb',cursor:'pointer',fontSize:13,fontWeight:500,color:'#374151'}}>{opt.label}</button>
                ))}
              </div>
            </div>
          ))}
      ...
      {isAdmin&&!standupEnabled&&(
        <div style={{marginBottom:14}}><button onClick={toggleStandup} style={{fontSize:11,color:'#6b7280',background:'none',border:'1px dashed #d1d5db',borderRadius:7,padding:'5px 13px',cursor:'pointer'}}>+ Turn on morning check-in</button></div>
      )}
      ...
      <div style={{display:'grid',gridTemplateColumns:'1.35fr 1fr',gap:12}}>
        <Card>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:10}}>
            {lbData.slice(0,3).map((item,i)=>{const c=PODIUM_COLORS[i];return(
              <div key={item.m.id} style={{background:c.bg,border:`0.5px solid ${c.border}`,borderRadius:9,padding:'10px',textAlign:'center'}}>
                ...
                {item.streak>0&&<div style={{fontSize:10,color:c.tx,opacity:.75,marginTop:2}}>🔥 {item.streak}d</div>}
              </div>
            )})}
          </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:11}}>
          <Card style={{flex:1}}>
            ...
            {isAdmin?members.map(m=>{const mt=tasks.filter(t=>t.uid===m.id);const done=mt.filter(t=>t.done).length;const p=mt.length?Math.round(done/mt.length*100):0;const bc=p===100?'#16a34a':p>50?'#d97706':'#dc2626';return(
              <div key={m.id} style={{marginBottom:9}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}>
                  <div style={{display:'flex',alignItems:'center',gap:6}}><div style={{width:20,height:20,borderRadius:'50%',background:'#dbeafe',display:'flex',alignItems:'center',justifyContent:'center',fontSize:7,fontWeight:600,color:'#1e40af'}}>{m.ini}</div><span style={{fontSize:11,fontWeight:500,color:'#111'}}>{m.name}</span></div>
                  <span style={{fontSize:10,color:bc,fontWeight:500}}>{done}/{mt.length}</span>
                </div>
                <div style={{height:3,background:'#f3f4f6',borderRadius:99}}><div style={{width:p+'%',height:'100%',background:bc,borderRadius:99}}/></div>
              </div>
              ...
              <div key={t.id} style={{display:'flex',alignItems:'center',gap:8,padding:'5px 0',borderBottom:'0.5px solid #f3f4f6'}}>
                ...
                <span style={{fontSize:12,flex:1,color:'#111',textDecoration:t.done?'line-through':'none',opacity:t.done?.5:1}}>{t.title}</span>
              </div>
            ))}
          </Card>
          ...
          <div style={{background:N,borderRadius:10,padding:'14px 16px'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
              <div style={{fontSize:10,fontWeight:500,color:'rgba(255,255,255,.5)',textTransform:'uppercase',letterSpacing:.5}}>Quote of the day</div>
              {isAdmin&&!editingQuote&&<button onClick={()=>{setQuoteDraft({text:quoteText,author:quoteAuthor});setEditingQuote(true)}} style={{background:'rgba(255,255,255,.12)',border:'.5px solid rgba(255,255,255,.2)',borderRadius:5,color:'rgba(255,255,255,.7)',fontSize:10,padding:'3px 9px',cursor:'pointer'}}>✏ Edit</button>}
            </div>
            {editingQuote?(
              <div>
                ...
                <div style={{display:'flex',gap:7}}>
                  ...
                  <button onClick={()=>setEditingQuote(false)} style={{background:'rgba(255,255,255,.1)',border:'.5px solid rgba(255,255,255,.2)',borderRadius:7,color:'rgba(255,255,255,.6)',fontSize:12,padding:'7px 14px',cursor:'pointer'}}>Cancel</button>
                </div>
              </div>
            ):(
              <>
      ...
      {editingAnnouncement&&(
        <Modal title="Post announcement" onClose={()=>setEditingAnnouncement(false)}>
          <Field label="Announcement text"><textarea value={announcementDraft} onChange={e=>setAnnouncementDraft(e.target.value)} rows={3} placeholder="e.g. Team meeting Friday at 2pm!" style={{...IS,resize:'none',lineHeight:1.5}}/></Field>
