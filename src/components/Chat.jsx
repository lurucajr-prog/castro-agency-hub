  ...
  return (
    <div style={{ borderTop:'1px solid var(--border)', background:'var(--surface)', padding:'10px 14px' }} onClick={e => e.stopPropagation()}>
      <div style={{ display:'flex', gap:8, marginBottom:8 }}>
        <input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key==='Enter') load(search) }} placeholder="Search GIFs… (press Enter)" style={{ flex:1, border:'1px solid var(--border-2)', borderRadius:7, padding:'6px 10px', fontSize:12, outline:'none', background:'var(--surface)', color:'var(--text-1)', fontFamily:'inherit' }} />
        <button onClick={() => load(search)} style={{ background:N, color:'#fff', border:'none', borderRadius:7, padding:'6px 12px', cursor:'pointer', fontSize:12 }}>Search</button>
        <button onClick={onClose} style={{ background:'none', border:'1px solid var(--border)', borderRadius:7, padding:'6px 10px', cursor:'pointer', fontSize:12, color:'var(--text-3)' }}>Cancel</button>
      </div>
      {loading ? (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:6, maxHeight:260, overflowY:'auto' }}>
          {gifs.map(g => (
            <div key={g.id} onClick={() => onSelect(g.images.original.url)} style={{ borderRadius:10, overflow:'hidden', cursor:'pointer', border:'2px solid transparent', transition:'border-color 0.12s' }}
              onMouseEnter={e => e.currentTarget.style.borderColor=N}
              onMouseLeave={e => e.currentTarget.style.borderColor='transparent'}>
              <img src={g.images.fixed_height_small.url} alt={g.title} style={{ width:'100%', height:110, objectFit:'cover', display:'block' }} />
            </div>
          ))}
        </div>
      )}
...
export default function Chat({ user }) {
  ...
  // ── Threading state ───────────────────────────────────────────
  const [threadMsg,          setThreadMsg]          = useState(null)   // parent message
  const [threadReplies,      setThreadReplies]      = useState([])
  const [threadText,         setThreadText]         = useState('')
  const [threadUploading,    setThreadUploading]    = useState(false)
  const [showThreadGif,      setShowThreadGif]      = useState(false)
  ...
  async function fetchAll() {
    ...
    // ── Subscriptions ──
    const msgCh = supabase.channel('chat_messages_v3')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, payload => {
        const msg = payload.new
        const ch  = msg.channel || 'main'
        if (!canSee(user.name, ch)) return
        setMessages(prev => { const u = { ...prev }; u[ch] = [...(u[ch] || []), msg]; return u })
        if (ch !== channelRef.current) setUnreadByChannel(prev => ({ ...prev, [ch]: (prev[ch] || 0) + 1 }))
        if (msg.uid !== user.id && msg.text?.includes(`@${user.name}`)) {
          ...
          supabase.from('notifications').insert({ to_uid: user.id, type: 'mention', title: `${sender?.name || 'Someone'} mentioned you in ${chLabel}`, body: msg.text, nav_target: 'chat', nav_channel: ch })
        }
        if (msg.uid !== user.id && user.role !== 'admin') {
          ...
          if (sender?.role === 'admin') {
            ...
            supabase.from('notifications').insert({ to_uid: user.id, type: 'announcement', title: `${sender.name} posted in ${chLabel}`, body: msg.text?.slice(0, 120) || '(shared an image)', nav_target: 'chat', nav_channel: ch })
          }
        }
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'messages' }, payload => {
        const msg = payload.new
        const ch  = msg.channel || 'main'
        setMessages(prev => { const u = { ...prev }; u[ch] = (u[ch] || []).map(m => m.id === msg.id ? msg : m); return u })
        // Update threadMsg if it's the parent being updated (e.g. reply_count changed)
        setThreadMsg(prev => prev?.id === msg.id ? { ...prev, ...msg } : prev)
        if (msg.pinned) {
          setPinnedMsgs(prev => { const u = { ...prev }; const list = u[ch] || []; u[ch] = list.find(m => m.id === msg.id) ? list.map(m => m.id === msg.id ? msg : m) : [msg, ...list]; return u })
        } else {
          setPinnedMsgs(prev => { const u = { ...prev }; u[ch] = (u[ch] || []).filter(m => m.id !== msg.id); return u })
        }
      })
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'messages' }, payload => {
        setMessages(prev => { const u = { ...prev }; Object.keys(u).forEach(ch => { u[ch] = u[ch].filter(m => m.id !== payload.old.id) }); return u })
        setPinnedMsgs(prev => { const u = { ...prev }; Object.keys(u).forEach(ch => { u[ch] = (u[ch] || []).filter(m => m.id !== payload.old.id) }); return u })
        if (threadMsg?.id === payload.old.id) closeThread()
      })
      .subscribe()

    const rxCh = supabase.channel('chat_reactions_v3')
    ...
    const voteCh = supabase.channel('poll_votes_v3')
    ...
    const typingCh = supabase.channel('typing_v3')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'typing_indicators' }, () => { fetchTypingUsers() })
      .subscribe()

    const readsCh = supabase.channel('reads_v2')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'channel_reads' }, payload => {
        const r = payload.new
        if (r) setChannelReads(prev => ({ ...prev, [r.uid]: { ...(prev[r.uid] || {}), [r.channel]: r.last_read_at } }))
      })
      .subscribe()

    return () => {
      supabase.removeChannel(msgCh); supabase.removeChannel(rxCh)
      supabase.removeChannel(voteCh); supabase.removeChannel(typingCh); supabase.removeChannel(readsCh)
    }
  }

  async function fetchTypingUsers() {
    const cutoff = new Date(Date.now() - STALE_TYPING).toISOString()
    const { data } = await supabase.from('typing_indicators').select('uid, channel, updated_at').neq('uid', user.id).gte('updated_at', cutoff)
    if (!data) return
    const grouped = {}
    data.forEach(t => {
      if (!grouped[t.channel]) grouped[t.channel] = []
      const prof = profiles.find ? profiles.find(p => p.id === t.uid) : null
      grouped[t.channel].push({ uid: t.uid, name: prof?.name || '...' })
    })
  ...
  async function openThread(msg) {
    // Close existing subscription if switching threads
    ...
    setThreadMsg(msg)
    setThreadText('')
    setShowThreadGif(false)

    const { data } = await supabase.from('thread_replies').select('*').eq('message_id', msg.id).order('created_at', { ascending: true })
    setThreadReplies(data || [])

    // Subscribe to replies for this thread
    threadSubRef.current = supabase.channel(`thread_${msg.id}`)
      .on('postgres_changes', {
        event: 'INSERT', schema: 'public', table: 'thread_replies',
        filter: `message_id=eq.${msg.id}`
      }, payload => {
        setThreadReplies(prev => [...prev, payload.new])
      })
      .on('postgres_changes', {
        event: 'DELETE', schema: 'public', table: 'thread_replies',
        filter: `message_id=eq.${msg.id}`
      }, payload => {
        setThreadReplies(prev => prev.filter(r => r.id !== payload.old.id))
      })
      .subscribe()
  }
  ...
  async function sendThreadReply(imageUrl = null) {
  ...
  async function uploadThreadImage(file) {
    ...
    const fileName = `thread-${user.id}-${Date.now()}.${ext}`
    const { error } = await supabase.storage.from('chat-images').upload(fileName, file, { contentType: file.type })
    if (error) { alert('Upload failed.'); setThreadUploading(false); return }
    const { data: { publicUrl } } = supabase.storage.from('chat-images').getPublicUrl(fileName)
    await sendThreadReply(publicUrl)
  ...
  function runSearch(q) {
    ...
    const results = all.filter(m => m.text?.toLowerCase().includes(q.toLowerCase()) && canSee(user.name, m.channel || 'main')).slice(0, 30)
  ...
  async function togglePin(msg) {
    ...
    const update = next ? { pinned: true, pinned_at: new Date().toISOString(), pinned_by: user.id } : { pinned: false, pinned_at: null, pinned_by: null }
    await supabase.from('messages').update(update).eq('id', msg.id)
  }

  // ── Edit ──────────────────────────────────────────────────────
  function openEdit(msg) { setEditingMsg(msg); setEditText(msg.text || '') }
  ...
  function handleTextChange(e) {
    ...
    if (match) {
      ...
      const exclude = CHANNELS.find(c => c.id === channel)?.exclude || []
      setMentionSuggestions(profiles.filter(p => p.id !== user.id && !exclude.includes(p.name) && p.name.toLowerCase().startsWith(q)).slice(0, 5))
  ...
  // ── Render a single message bubble (used in both main chat and thread original) ──
  function MessageBubble({ item, isMe, isThread = false }) {
    const p = getProfile(item.uid)
    return (
      <>
        {item.image_url && (
          <div style={{ marginBottom: item.text ? 6 : 0 }}>
            <img src={item.image_url} alt="shared" onClick={() => setLightbox(item.image_url)} style={{ maxWidth: isThread ? 220 : 260, maxHeight: 200, borderRadius: 10, cursor: 'zoom-in', display: 'block', border: '1px solid var(--border)', objectFit: 'cover' }} />
          </div>
        )}
        {item.text && (
          <div style={{ background: isMe ? N : 'var(--surface-3)', color: isMe ? '#fff' : 'var(--text-1)', padding: '9px 13px', borderRadius: isMe ? '12px 2px 12px 12px' : '2px 12px 12px 12px', fontSize: 13, lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
            {isThread ? item.text : renderText(item.text)}
          </div>
        )}
      </>
    )
  ...
  return (
    <>
      <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
        ...
        {/* ── Channel tabs ── */}
        <div style={{ borderBottom:'1px solid var(--topbar-border)', background:'var(--topbar-bg)', padding:'0 16px', display:'flex', alignItems:'center', flexShrink:0 }}>
          <div style={{ display:'flex', flex:1 }}>
            {accessibleChannels.map(ch => {
              const unread = unreadByChannel[ch.id] || 0
              return (
                <button key={ch.id} onClick={() => setChannel(ch.id)} style={{ padding:'11px 14px', border:'none', borderBottom: channel===ch.id ? `2px solid ${N}` : '2px solid transparent', background:'none', cursor:'pointer', fontSize:13, fontWeight: channel===ch.id ? 600 : 400, color: channel===ch.id ? N : 'var(--text-3)', marginBottom:-1, display:'flex', alignItems:'center', gap:6 }}>
                  {ch.label}
                  {unread > 0 && channel !== ch.id && (
                    <span style={{ background:R, color:'#fff', borderRadius:99, fontSize:9, fontWeight:700, padding:'1px 5px', minWidth:16, textAlign:'center' }}>{unread > 9 ? '9+' : unread}</span>
                  )}
                </button>
              )
            })}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6, paddingLeft:10 }}>
            {currentPinned.length > 0 && (
              <button onClick={() => setShowPinned(o => !o)} style={{ fontSize:11, color: showPinned ? N : 'var(--text-3)', background: showPinned ? 'var(--primary-light)' : 'none', border:`1px solid ${showPinned ? N : 'var(--border)'}`, borderRadius:7, padding:'4px 10px', cursor:'pointer', display:'flex', alignItems:'center', gap:4 }}>
                📌 {currentPinned.length}
              </button>
            )}
            <button onClick={() => setSearchOpen(o => !o)} style={{ fontSize:13, color: searchOpen ? N : 'var(--text-3)', background: searchOpen ? 'var(--primary-light)' : 'none', border:`1px solid ${searchOpen ? N : 'var(--border)'}`, borderRadius:7, padding:'4px 10px', cursor:'pointer' }}>🔍</button>
          </div>
        </div>
        ...
        {searchOpen && (
          <div style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'10px 16px' }} onClick={e => e.stopPropagation()}>
            <div style={{ display:'flex', gap:8, alignItems:'center' }}>
              <input ref={searchRef} value={searchQuery} onChange={e => runSearch(e.target.value)} placeholder="Search messages across all channels…" style={{ flex:1, border:'1px solid var(--border-2)', borderRadius:8, padding:'7px 12px', fontSize:13, outline:'none', background:'var(--surface)', color:'var(--text-1)', fontFamily:'inherit' }} />
        ...
        {showPinned && currentPinned.length > 0 && (
          <div style={{ background:'var(--warning-light)', borderBottom:'1px solid #fcd34d', padding:'8px 16px', maxHeight:140, overflowY:'auto', flexShrink:0 }}>
              ...
              return (
                <div key={msg.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:5, cursor:'pointer' }} onClick={() => jumpToMessage(msg)}>
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <span style={{ fontSize:11, color:'#78350f' }}>{msg.text || '(image)'}</span>
                  </div>
                  {isAdmin && <button onClick={e => { e.stopPropagation(); togglePin(msg) }} style={{ fontSize:10, color:'#92400e', background:'none', border:'none', cursor:'pointer', flexShrink:0, marginLeft:8 }}>Unpin</button>}
                </div>
              )
            })}
          </div>
        ...
        {/* ── Horizontal flex: main chat + thread panel ── */}
        <div style={{ flex:1, display:'flex', overflow:'hidden' }}>
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
            ...
            <div style={{ flex:1, overflowY:'auto', padding:'14px 18px', display:'flex', flexDirection:'column', gap:10 }}
              ...
              {grouped.length === 0 && (
                <div style={{ textAlign:'center', color:'var(--text-4)', fontSize:13, marginTop:40 }}>
                  No messages yet in {CHANNELS.find(c=>c.id===channel)?.label}. Say something! 👋
                </div>
              )}
              ...
              {grouped.map((item, idx) => {
                  ...
                  return (
                    <div key={item.id} id={`msg-${item.id}`} style={{ maxWidth:360, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'12px 14px', transition:'background 0.4s' }}>
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
                        ...
                        {expLabel && <span style={{ fontSize:10, fontWeight:500, color: expired?'var(--danger)':'#92400e', background: expired?'var(--danger-light)':'var(--warning-light)', padding:'2px 7px', borderRadius:99 }}>{expLabel}</span>}
                      </div>
                      ...
                      {poll.options.map((opt, i) => {
                        ...
                        return (
                          <div key={i} onClick={() => !myVote && !expired && vote(poll.id, i)} style={{ marginBottom:7, cursor: (!myVote && !expired) ? 'pointer' : 'default' }}>
                ...
                const isThreadOpen = threadMsg?.id === item.id

                return (
                  <div key={item.id} id={`msg-${item.id}`}
                    onMouseEnter={() => setHoveredMsg(item.id)}
                    onMouseLeave={() => setHoveredMsg(null)}
                    style={{ display:'flex', gap:9, flexDirection: isMe?'row-reverse':'row', alignItems:'flex-start', borderRadius:8, padding:'2px 4px', margin:'0 -4px', transition:'background 0.4s', background: isThreadOpen ? 'var(--primary-light)' : 'transparent' }}>
                    ...
                    <div style={{ maxWidth:'72%' }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:6, flexDirection: isMe?'row-reverse':'row', marginBottom:3 }}>
                        ...
                        {isHovered && (
                          <div style={{ display:'flex', gap:3 }}>
                            {/* Reply button */}
                            <button onClick={() => openThread(item)} style={{ border:'none', background:'none', cursor:'pointer', fontSize:11, color: isThreadOpen ? N : 'var(--text-4)', padding:'1px 4px', borderRadius:4, fontWeight: isThreadOpen?600:400 }} title="Reply in thread">💬 Reply</button>
                            {isMe && item.text && <button onClick={() => openEdit(item)} style={{ border:'none', background:'none', cursor:'pointer', fontSize:11, color:'var(--text-4)', padding:'1px 3px', borderRadius:4 }} title="Edit">✏️</button>}
                            {isAdmin && <button onClick={() => togglePin(item)} style={{ border:'none', background:'none', cursor:'pointer', fontSize:11, color: item.pinned?'#d97706':'var(--text-4)', padding:'1px 3px', borderRadius:4 }} title={item.pinned?'Unpin':'Pin'}>📌</button>}
                            {(isMe || isAdmin) && <button onClick={() => setConfirmDeleteMsg(item)} style={{ border:'none', background:'none', cursor:'pointer', fontSize:11, color:'#ef4444', padding:'1px 3px', borderRadius:4 }} title="Delete">🗑</button>}
                          </div>
                        )}
                      </div>
                      ...
                      {isEditing ? (
                        <div>
                          <textarea value={editText} onChange={e => setEditText(e.target.value)} onKeyDown={e => { if (e.key==='Enter'&&!e.shiftKey){e.preventDefault();saveEdit()} if(e.key==='Escape'){setEditingMsg(null);setEditText('')} }} autoFocus rows={2}
                      ...
                      {/* Reply count badge */}
                      {(item.reply_count > 0) && (
                        <button onClick={() => openThread(item)} style={{ display:'flex', alignItems:'center', gap:5, marginTop:5, border:'none', background:'none', cursor:'pointer', padding:0 }}>
                          <span style={{ fontSize:13 }}>💬</span>
                          <span style={{ fontSize:11, color: isThreadOpen ? N : 'var(--primary)', fontWeight:600, textDecoration: isThreadOpen?'none':'underline' }}>
                            {item.reply_count} {item.reply_count === 1 ? 'reply' : 'replies'}
                          </span>
                          {item.last_reply_at && (
                            <span style={{ fontSize:10, color:'var(--text-4)' }}>· {formatRelative(item.last_reply_at)}</span>
                          )}
                        </button>
                      )}
              ...
              {(() => {
                ...
                const seenBy = profiles.filter(p => p.id !== user.id && canSee(p.name, channel) && channelReads[p.id]?.[channel] && new Date(channelReads[p.id][channel]) >= new Date(lastMsg.created_at))
            ...
            {/* Poll form */}
            {showPollForm && (
              <div style={{ padding:'10px 16px', borderTop:'1px solid var(--border)', background:'var(--surface-2)', flexShrink:0 }}>
            ...
            {/* GIF picker */}
            {showGifPicker && <GifPicker onSelect={gifUrl => { setShowGifPicker(false); supabase.from('messages').insert({ uid: user.id, text: '', image_url: gifUrl, channel }) }} onClose={() => setShowGifPicker(false)} />}
            ...
            <div style={{ padding:'10px 16px', borderTop:'1px solid var(--border)', background:'var(--topbar-bg)', flexShrink:0 }}>
              ...
              <div style={{ display:'flex', gap:8, alignItems:'flex-end', position:'relative' }}>
                ...
                <textarea ref={textareaRef} value={text} onChange={handleTextChange} onKeyDown={onKey} onPaste={handlePaste}
                  placeholder={`Message ${CHANNELS.find(c=>c.id===channel)?.label}… (@ to mention)`}
          ...
          {/* ── Right: Thread panel ── */}
          {threadMsg && (
            <div style={{ width:380, borderLeft:'1px solid var(--border)', display:'flex', flexDirection:'column', background:'var(--surface)', flexShrink:0 }}>
              ...
              <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
                <div>
                  <div style={{ fontSize:14, fontWeight:600, color:'var(--text-1)' }}>Thread</div>
                  <div style={{ fontSize:11, color:'var(--text-3)' }}>
                    {CHANNELS.find(c => c.id === channel)?.label} · {threadReplies.length} {threadReplies.length===1?'reply':'replies'}
                  </div>
                </div>
                <button onClick={closeThread} style={{ border:'none', background:'none', fontSize:22, cursor:'pointer', color:'var(--text-4)', lineHeight:1, padding:'0 2px' }}>×</button>
              </div>

              {/* Original message */}
              <div style={{ padding:'14px 16px', borderBottom:'1px solid var(--border)', background:'var(--surface-2)', flexShrink:0 }}>
                {(() => {
                  const p    = getProfile(threadMsg.uid)
                  const isMe = threadMsg.uid === user.id
                  return (
                    <div style={{ display:'flex', gap:9, alignItems:'flex-start' }}>
                      ...
                      <div>
                        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4 }}>
                          <span style={{ fontSize:11, fontWeight:600, color:'var(--text-2)' }}>{isMe ? 'You' : p.name}</span>
                          <span style={{ fontSize:10, color:'var(--text-4)' }}>{formatTime(threadMsg.created_at)}</span>
                        </div>
                        <MessageBubble item={threadMsg} isMe={isMe} isThread />
                      </div>
                    </div>
                  )
                })()}
              ...
              <div style={{ flex:1, overflowY:'auto', padding:'12px 16px', display:'flex', flexDirection:'column', gap:10 }}>
                {threadReplies.length === 0 && (
                  <div style={{ textAlign:'center', color:'var(--text-4)', fontSize:12, marginTop:20, lineHeight:1.7 }}>
                    No replies yet.<br />Be the first to reply!
                  </div>
                )}

                {threadReplies.map(reply => {
                  const isMe     = reply.uid === user.id
                  const p        = getProfile(reply.uid)
                  const canDel   = isMe || isAdmin
                  const isHov    = hoveredReply === reply.id

                  return (
                    <div key={reply.id}
                      onMouseEnter={() => setHoveredReply(reply.id)}
                      onMouseLeave={() => setHoveredReply(null)}
                      style={{ display:'flex', gap:8, flexDirection: isMe?'row-reverse':'row', alignItems:'flex-start' }}>
                      <div style={{ width:26, height:26, borderRadius:'50%', flexShrink:0, background: p.role==='admin'?R:'var(--primary-mid)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, fontWeight:600, color: p.role==='admin'?'#fff':'#1e40af' }}>{p.ini}</div>
                      <div style={{ maxWidth:'82%' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:5, flexDirection: isMe?'row-reverse':'row', marginBottom:3 }}>
                          <span style={{ fontSize:11, fontWeight:600, color:'var(--text-2)' }}>{isMe ? 'You' : p.name}</span>
                          <span style={{ fontSize:10, color:'var(--text-4)' }}>{formatTime(reply.created_at)}</span>
                          {canDel && isHov && (
                            <button onClick={() => setConfirmDeleteReply(reply)} style={{ border:'none', background:'none', cursor:'pointer', fontSize:10, color:'#ef4444', padding:'1px 2px' }} title="Delete">🗑</button>
                          )}
                        </div>
                        {reply.image_url && (
                          <div style={{ marginBottom: reply.text ? 5 : 0 }}>
                            <img src={reply.image_url} alt="reply img" onClick={() => setLightbox(reply.image_url)} style={{ maxWidth:220, maxHeight:180, borderRadius:10, cursor:'zoom-in', display:'block', border:'1px solid var(--border)', objectFit:'cover' }} />
                          </div>
                        )}
                        {reply.text && (
                          <div style={{ background: isMe?N:'var(--surface-3)', color: isMe?'#fff':'var(--text-1)', padding:'8px 12px', borderRadius: isMe?'12px 2px 12px 12px':'2px 12px 12px 12px', fontSize:13, lineHeight:1.55, whiteSpace:'pre-wrap', wordBreak:'break-word' }}>
                            {reply.text}
                          </div>
                        )}
                      </div>
              ...
              {showThreadGif && (
                <GifPicker
                  onSelect={async gifUrl => { setShowThreadGif(false); await sendThreadReply(gifUrl) }}
                  onClose={() => setShowThreadGif(false)}
                />
              )}

              {/* Thread reply input */}
              <div style={{ padding:'10px 14px', borderTop:'1px solid var(--border)', flexShrink:0, background:'var(--topbar-bg)' }}>
                {threadUploading && <div style={{ fontSize:11, color:'var(--text-3)', marginBottom:5 }}>⏳ Uploading…</div>}
                <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                  <button onClick={() => threadFileRef.current?.click()} style={{ width:30, height:30, borderRadius:'50%', border:'1px solid var(--border)', background:'var(--surface)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, color:'var(--text-3)', flexShrink:0 }}>📎</button>
                  <button onClick={() => setShowThreadGif(s => !s)} style={{ width:30, height:30, borderRadius:'50%', border:`1px solid ${showThreadGif?N:'var(--border)'}`, background: showThreadGif?'var(--primary-light)':'var(--surface)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:700, color: showThreadGif?N:'var(--text-3)', flexShrink:0 }}>GIF</button>
                  <input ref={threadFileRef} type="file" accept="image/*" style={{ display:'none' }} onChange={async e => { const f = e.target.files[0]; if (f) await uploadThreadImage(f); e.target.value='' }} />
                  <input
                    value={threadText}
                    ...
                    onPaste={async e => {
                      ...
                      e.preventDefault()
                      const f = item.getAsFile()
                      if (f) await uploadThreadImage(new File([f], 'paste.png', { type: f.type }))
                    }}
                    placeholder="Reply in thread…"
                    disabled={threadUploading}
                    style={{ flex:1, fontSize:12, padding:'8px 12px', border:'1px solid var(--border)', borderRadius:16, background:'var(--surface-2)', color:'var(--text-1)', outline:'none', fontFamily:'inherit' }}
                  />
                  <button onClick={() => sendThreadReply()} disabled={threadUploading} style={{ background:N, color:'#fff', border:'none', borderRadius:16, padding:'8px 14px', fontSize:12, fontWeight:500, cursor:'pointer', flexShrink:0 }}>Reply</button>
                </div>
              </div>
            </div>
          )}
      ...
      {confirmDeleteMsg && (
        <ConfirmModal
          ...
          message="This permanently removes the message and all its replies for everyone."
          confirmLabel="Delete"
          danger
          onConfirm={async () => { await supabase.from('messages').delete().eq('id', confirmDeleteMsg.id); setConfirmDeleteMsg(null) }}
      ...
      {confirmDeleteReply && (
        <ConfirmModal
          ...
          message="This permanently removes the reply."
          confirmLabel="Delete"
          danger
          onConfirm={() => deleteThreadReply(confirmDeleteReply)}
